...
struct myExampleTask {
  ...
  void init(InitContext const&)
  {
    ...
    const AxisSpec axisCounter{1, 0, 1, "Counter"};
    const AxisSpec axisDeltaPt{100, -1.0, +1.0, "#Delta(p_{T})"};
    const AxisSpec axisRecoColl{10, 0, 10, ""};
    const AxisSpec axisMultipCorr{45, 0, 45, "Number of tracks"};
    ...
    histos.add("numberOfRecoCollisions", "numberOfRecoCollisions", kTH1F, {axisRecoColl});
    histos.add("multiplicityCorrelation", "multiplicityCorrelation", kTH2F, {axisMultipCorr, axisMultipCorr});
  }
  ...
  void processSim(aod::McCollision const& mcCollision, soa::SmallGroups<soa::Join<aod::McCollisionLabels, aod::Collisions>> const& collisions, aod::McParticles const& mcParticles, myFilteredTracks const& tracks)
  {
    ...
    for (auto& collision : collisions) {
      auto groupedTracks = tracks.sliceBy(perCollision, collision.globalIndex()); // size of grouped tracks may help in understanding why event was split! 
